...
class CNNFaceTracker:
    def __init__(self):
        rospy.init_node('cnn_face_tracker')
        self.pub = rospy.Publisher('/cnn_face_status', String, queue_size=10)
        ...
        rospy.loginfo("CNN Face Tracker iniciado")
        ...
        try:
            while not rospy.is_shutdown():
                ...
                if current_time - self.last_time >= 1.0:
                    ...
                    # Publicar métricas (formato igual que HOG)
                    msg = f"CNN|FPS:{self.fps:.1f}|Faces:{self.face_count}|CPU:{self.cpu_usage:.1f}|MEM:{self.mem_usage:.1f}"
